...
export abstract class GutterMarker extends RangeValue {
  ...
  // eslint-disable-next-line ember/classic-decorator-hooks, @typescript-eslint/no-unused-vars
  destroy(dom: Node) {
...
const gutterView = ViewPlugin.fromClass(
  class {
    ...
    constructor(readonly view: EditorView) {
      ...
      this.dom.style.backgroundColor = '#ff000070';
      ...
      if (prev == cur) {
        ...
        for (const conf of cur) {
          ...
          if (known < 0) {
            ...
            if (this.gutters[known] !== undefined) {
  ...
  constructor(
    ...
    for (const prop in config.domEventHandlers) {
      this.dom.addEventListener(prop, (event: Event) => {
        ...
        if ((config.domEventHandlers[prop] || function () {})(view, line, event)) event.preventDefault();
      });
    }
