...
export default [
  ...
  new DiffBasedExampleDocument({
    diff: [
      ...
      ' ',
      ' const Mode = enum { read, write, read_write };',
      ...
      ' ',
      '     const slice = try parseNumbers(allocator, "1,2,3");',
      '     defer allocator.free(slice);',
      '     for (slice) |n| std.debug.print("{d} ", .{n});',
      '     std.debug.print("\\n", .{});',
      ' ',
      '     const mode: Mode = .read_write;',
      '     switch (mode) {',
      '         .read => std.debug.print("read\\n", .{}),',
      '         .write => std.debug.print("write\\n", .{}),',
      '-        .read_write => std.debug.print("read_write\\n", .{}),',
      '+        .read_write => std.debug.print("read+write\\n", .{}),',
      '     }',
      ' ',
      '     var buf = Buffer.init("hello");',
      '     while (buf.next()) |byte| {',
      ...
      ' }',
      ' ',
      ' fn parseNumbers(allocator: std.mem.Allocator, input: []const u8) ParseError![]const u32 {',
    ...
    highlightedRanges: [
      ...
      { startLine: 6, endLine: 20 },
      { startLine: 38, endLine: 39 },
      { startLine: 48, endLine: 52 },
    ],
    collapsedRanges: [
      { startLine: 1, endLine: 5 },
      { startLine: 22, endLine: 44 },
