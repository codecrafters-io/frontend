...
export default class RepositoryModel extends Model {
  ...
  get activatedCourseExtensions() {
    return [
      ...new Set([...this.extensionActivations].sort((a, b) => compare(a.activatedAt, b.activatedAt)).map((activation) => activation.extension)),
    ];
  }
  ...
  get highestCompletedStage() {
    ...
    return [...this.courseStageCompletions].sort((a, b) => compare(a.courseStage.position, b.courseStage.position)).at(-1)?.courseStage;
  }
  ...
  courseStageCompletionFor(courseStage: CourseStageModel) {
    return [...this.courseStageCompletions.filter((item) => item.courseStage === courseStage)].sort((a, b) =>
