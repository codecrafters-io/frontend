...
module('Unit | Utility | time-formatting', function () {
  test('it formats time remaining correctly', function (assert) {
    ...
    assert.strictEqual(
      formatTimeLeft(new Date('2024-01-01T02:34:56Z'), currentTime),
      '02h:34m:56s',
    ...
    assert.strictEqual(
      formatTimeLeft(new Date('2024-01-01T00:00:45Z'), currentTime),
      '45s',
    ...
    assert.strictEqual(
      formatTimeLeft(new Date('2023-12-31T23:59:59Z'), currentTime),
      '00s',
      'returns 00s for expired time'
    );

    // Test zero seconds
    assert.strictEqual(
      formatTimeLeft(new Date('2024-01-01T00:00:00Z'), currentTime),
      '00s',
      'formats zero seconds correctly'
    );
  });
});
