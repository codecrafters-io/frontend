...
module('Acceptance | course-page | earn-badge', function (hooks) {
  ...
  test('passing first stage shows badges for staff users', async function (assert) {
    ...
    assert.strictEqual(coursePage.header.stepName, 'Bind to a port', 'first stage is active');
    ...
    assert.strictEqual(coursePage.header.stepName, 'Bind to a port', 'first stage is still active');
