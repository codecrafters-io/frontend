...
async function waitForFocusedContinueButton() {
  await waitUntil(() => document.activeElement instanceof HTMLElement && document.activeElement.matches('[data-test-continue-button]'), {
    timeout: 5000,
  });
}

async function assertContinueButtonIsFocused(assert, message = 'Continue button is focused') {
  await waitForFocusedContinueButton();
